...
namespace s2t
{
	namespace common
	{
		...
		struct gup_data_type
		{
			...
			// copy gpu data to host and returs a T ptr
			// deletion of this T* has to be done by caller
			size_t data_at_host(T** local_ptr) const
			{
				size_t N = shape[0] * shape[1] * shape[2] * shape[3];
				cudaMemcpy(*local_ptr, ptr, N*sizeof(T), cudaMemcpyDeviceToHost); 
				return N;
			}
		...
		typedef gup_data_type<float_t> const_gpu_float_array;
